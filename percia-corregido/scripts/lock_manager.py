...
class LockManager:
    ...
    def _write_lock(self, lock_info: LockInfo) -> None:
        """Escribe información del lock de forma atómica"""
    ...
    def _save_queue(self, queue: List[QueueItem]) -> None:
        """Guarda la cola de forma atómica"""
    ...
    def _log_lock_event(self, event_type: str, lock_info: Optional[LockInfo], details: str = "") -> None:
        ...
        if self.lock_history_file.exists():
            try:
                with open(self.lock_history_file, 'r') as f:
                    history = json.load(f)
            except:
                history = []
        ...
        with open(self.lock_history_file, 'w') as f:
    ...
    def acquire_global_lock(
        ...
        """
        Adquiere lock global del sistema con TTL
        
        Args:
            ia_id: Identificador de la IA que solicita el lock
            operation_type: Tipo de operación (proposal, challenge, etc.)
            timeout: Tiempo máximo de espera (default: TTL)
            force_if_expired: Si True, fuerza liberación de locks expirados
        
        Returns:
            bool: True si se adquirió el lock
        
        Raises:
            TimeoutError: Si no se puede adquirir el lock en el tiempo especificado
        """
        ...
        while True:
            ...
            if current_lock is None:
                # Lock libre, intentar adquirir
                break
            
            # Lock existe, verificar si está expirado
            if current_lock.is_expired():
                if force_if_expired:
                    logger.warning(f"Lock expirado de {current_lock.ia_id}, liberando...")
                    self._log_lock_event("expired_release", current_lock, "Lock expirado y liberado")
                    self._force_release_lock()
                    break
            
            # Lock existe y no está expirado, verificar si el proceso está vivo
            if not self._is_process_alive(current_lock.pid):
                logger.warning(f"Proceso {current_lock.pid} muerto, liberando lock huérfano...")
                self._log_lock_event("orphan_release", current_lock, "Proceso muerto detectado")
                self._force_release_lock()
                break
            
            # Lock válido, esperar
            attempts += 1
            logger.debug(f"Lock ocupado por {current_lock.ia_id}, intento {attempts}...")
        ...
        logger.info(f"Lock adquirido por {ia_id} (owner: {self._owner_id})")
        return True
    ...
    def release_global_lock(self, force: bool = False) -> bool:
        ...
        if not force and self._owner_id and current_lock.owner_id != self._owner_id:
            logger.warning(f"Intento de liberar lock ajeno")
            return False
    ...
    def refresh_lock(self) -> bool:
        """Renueva el TTL del lock actual (heartbeat)"""
        current_lock = self._read_lock()
    ...
    def _watchdog_loop(self) -> None:
        """Loop del watchdog para limpiar locks expirados"""
        while self._watchdog_running:
            try:
                ...
                if current_lock:
                    if current_lock.is_expired():
                        logger.warning(f"Watchdog: Lock expirado de {current_lock.ia_id}")
                        self._log_lock_event("watchdog_expired", current_lock)
                        self._force_release_lock()
                    elif not self._is_process_alive(current_lock.pid):
                        logger.warning(f"Watchdog: Proceso {current_lock.pid} muerto")
